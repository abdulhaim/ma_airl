...
# size_r = 0.9
# SIZE = [1 * size_r, 1.5 * size_r, 2 * size_r]

DENSITY = [1, 2]

# STRENGTH = [300 / 100, 600 / 100, 150 / 100]
STRENGTH = [150 / 5, 300 / 5, 600 / 5, 450 / 5] #orig
STRENGTH = [150 / 5, 300 / 5, 450 / 5, 600 / 5]
# STRENGTH = [150 / 5, 300 / 5, 600 / 5, 560 / 5, 360 / 5]
...
class Belief:
    def __init__(self, room_dim, world_center, agent_id, self_state, num_entities, n_particles):
        ...
        self.particles = [[None for _ in range(num_entities)] for _ in range(n_particles)]
        x,y = np.meshgrid(list(range(int(world_center[0]-(room_dim[0])//2+2),
                                     int(world_center[0]+(room_dim[0])//2-1))),\
                          list(range(int(world_center[1]-(room_dim[1])//2+2),
        ...
        for world_state in self.particles:
            for entity_idx in range(num_entities):
                if entity_idx == agent_id:
                    ...
                    p['angle'] = 0. #math.radians(np.random.uniform(-360,360))
                    p['vel'] = [0.,0.] #lin.x, lin.y
                    p['angleVel'] = 0.
                    p['attached'] = None
    ...
    def sample_dist_state(self):
        ...
        all_pos = [np.array([[np.floor(p['pos'][0]),np.floor(p['pos'][1])] for p in world_state]).flatten()\
                    for world_state in self.particles]
...
class MyContactListener(b2ContactListener):
    ...
    def agent_agent_remove(self,agent1,agent2):
        #update both agents
        if agent2[0] not in agent1[1][agent1[0][1]] or\
           agent1[0] not in agent2[1][agent2[0][1]]:
...
class Maze_v1(gym.Env):
    ...
    def setup(self, env_id=0, max_episode_length=100, record_path=None, contact_listen=False, \
              agent_order=None, item_order=None, landmark_color_order=None, flip_env=None, rotate_env=False):
        ...
        self.doors_pos = [(16, self.room_dim[1] / 4 + 12),
                          ...
                          (self.room_dim[0] / 4 + 16, 12)] #middle door pos
        ...
        self.doors_pos_start = [(16, (self.room_dim[1] // 2) - self.doors_size[0] + 12),
        ...
        self.tmp_trajectories = [[] for _ in range(self.num_agents + self.num_items + 1)]

        room_dim, world_center = self.room_dim, self.room.position
        x,y = np.meshgrid(list(range(int(world_center[0]-(room_dim[0])//2 + 2),
                                     int(world_center[0]+(room_dim[0])//2 - 2))),\
                          list(range(int(world_center[1]+(room_dim[1])//2 - 2),
                                     int(world_center[1]-(room_dim[1])//2 + 2), -1)))
        self.all_init_pos = list(zip(*(x.flatten(),y.flatten())))
        # add agents
        for agent_id in range(self.num_agents):
            if self.use_all_init_grid_pos:
                gx, gy = self.all_init_pos[self.init_positions[agent_id]][0], self.all_init_pos[self.init_positions[agent_id]][1]
            ...
            random.seed(datetime.now())
            body.beliefs = Belief(room_dim=self.room_dim, world_center=self.room.position, agent_id=agent_id, \
                                  self_state=_get_state_from_body(body), num_entities=self.num_agents+self.num_items, \
                                  n_particles=self.n_particles)
            self.agents.append(body)
            self.trajectories[agent_id] = [(x, y, 0, 0, self.init_agent_angles[agent_id])]
        ...
        for item_id in range(self.num_items):
            ...
            if self.use_all_init_grid_pos:
                gx, gy = self.all_init_pos[self.init_positions[index]][0], self.all_init_pos[self.init_positions[index]][1]
                x = random.uniform(gx, gx+1)
                y = random.uniform(gy-1, gy)
            else:
    ...
    def start(self, replay=False, record_dir=None):
        ...
        self.observation_space = [None for agent in range(self.num_agents)] #updated in update_observations()
        ...
        if self.use_all_init_grid_pos and record_dir is not None:
            ...
            visibility_X_agent_vs = [[[ag.GetWorldPoint(localPoint=v)[0],ag.GetWorldPoint(localPoint=v)[1]]\
                                       for v in ag.fixtures[0].shape.vertices] \
                                       for ag in self.agents]
            # print('visibility_X_agent_vs',visibility_X_agent_vs)
            visibility_X_item_pos_r = [[it['pos'][0],it['pos'][1],SIZE[s]] for it,s in zip(self.item_states,self.sizes[self.num_agents:])]
            visibility_Y = [('agent',1) in self.agents[0].last_observations, ('agent',0) in self.agents[1].last_observations]
            path = record_dir + '/visibility.pik'
            pickle.dump([visibility_X_door_pos, visibility_X_agent_vs, visibility_X_item_pos_r, visibility_Y], \
                        open(path, 'wb'))
    ...
    def reset_history(self):
        ...
        self.trajectories = [[(state['pos'][0], state['pos'][1], state['vel'][0], state['vel'][1], state['angle'])] for state in self.agent_states+ self.item_states]
    ...
    def check_attachable(self, agent_id, item_id=None, threshold=0.5):
        ...
        if self.action_space_types[agent_id] == 2 or self.attached[agent_id] is not None:
            return False
        ...
        if item_id is not None:
            all_dist = [_get_dist(pos, self.item_states[item_id]['pos']) for pos in all_agent_pos]
            ...
            for item_id in range(self.num_items):
                ...
                if cur_dist < SIZE[self.sizes[self.num_agents + item_id]] + threshold and cur_dist < min_dist:
                    min_dist = cur_dist
                    selected_item_id = item_id
                    selected_agent_anchor_idx = agent_anchor_idx
    ...
    def get_action_space(self, agent_id):
        ...
        if self.goals[agent_id][0] in ['RO', 'GE', 'TE']:
            ...
            num_actions = min(num_actions, len(self.action_space) - 1)
        elif self.goals[agent_id][0] == 'RA' or (self.goals[agent_id][0] == 'LMA' and self.goals[agent_id][1] == agent_id):
            return ['turnleft', 'turnright', 'up', 'stop', 'noforce']
    ...
    def send_action(self, agent_id, action):
        ...
        if action.startswith('turn'):
            ...
            size_factor = (SIZE[self.sizes[agent_id]] / 0.8) ** 2 #size_factor=1.0 for size=0.8
            if action == 'turnleft':
                # self.agents[agent_id].ApplyTorque(min(df * 1.0, 300), True)
                self.agents[agent_id].ApplyTorque(min(df, 300) * size_factor, True)
            else:
                # self.agents[agent_id].ApplyTorque(-min(df * 1.0, 300), True)
                self.agents[agent_id].ApplyTorque(-min(df, 300) * size_factor, True) #TODO for human game and in general the correct direction
    ...
    def step(self, actions, replay=False, record_dir=None):
        ...
        for agent_id, state in enumerate(self.agent_states):
            agent_pos, agent_vel, agent_angle = state['pos'], state['vel'], state['angle']
        ...
        for item_id, state in enumerate(self.item_states):
            ...
            self.trajectories[item_id + self.num_agents].append((item_pos[0], item_pos[1],
                                                       item_vel[0], item_vel[1], item_angle))
    ...
    def sample_belief_state(self, agent_id, action):
        ...
        if curr_belief is None: #first call to this func, no prev state. choose random belief.
            ...
            next_belief = self.transition(agent_id, curr_belief, action=action, expected_action=None)
            for obs in self.agents[agent_id].last_observations.keys():
                ...
                if obs[0] == 'agent':
                    ...
                    next_belief[self.num_agents+idx]['pos'] = self.item_states[idx]['pos']
                    next_belief[self.num_agents+idx]['vel'] = self.item_states[idx]['vel']
                    next_belief[self.num_agents+idx]['angle'] = self.item_states[idx]['angle']
                    next_belief[self.num_agents+idx]['angleVel'] = self.item_states[idx]['angleVel']
                    next_belief[self.num_agents+idx]['attached'] = self.item_states[idx]['attached']
            ...
            if not matches[0]: #for each entity that does not match
                ...
                for entity in matches[1]:
                    if entity==('agent',agent_id) or entity in self.agents[agent_id].last_observations:
                        ...
                        entity_particles = [state[entity_id] for state in beliefs.particles]
                        entity_grid_pos = [self.world_point_to_grid_cell(p['pos'][0],p['pos'][1]) for p in entity_particles]
                        #unique_pos has grid cells
                        unique_pos, inv_idx, cnt = np.unique(entity_grid_pos, return_inverse=True, return_counts=True, axis=0)
                        idx = [s[1] for s in sorted(zip(cnt,list(range(len(unique_pos)))),reverse=True)]
                        ...
                        for p_idx in particles_by_freq:
                            ...
                            next_resampled[entity_id] = copy.deepcopy(beliefs.particles[int(p_idx)][entity_id])
                            # next_resampled = self.valid_belief(agent_id, next_resampled)
                            if self.belief_matches_observation(agent_id, next_resampled)[0]:
    ...
    def get_belief_certainty(self, agent_id):
        ...
        for other_agent_id in range(self.num_agents):
            ...
            grid_cells = self.get_perimeter_grid_cells('agent', other_agent_id, self.tmp_agents[other_agent_id])
            scores.append(np.average([self.agents[agent_id].certainty[c[0],c[1]] for c in grid_cells]))
        for item_id in range(self.num_items):
            grid_cells = self.get_perimeter_grid_cells('item', item_id, self.tmp_item_states[item_id])
            item_score = np.average([self.agents[agent_id].certainty[c[0],c[1]] for c in grid_cells])
    ...
    def get_state(self, agent_id=None):
        ...
        if agent_id is None:
            ...
            return [_get_state_from_body(agent) for agent in self.agents] + \
                   [_get_state_from_body(item) for item in self.items], \
                   [self.agents[i].certainty.copy() if i == agent_id or self.full_obs[agent_id] else np.zeros(self.room_dim) for i in range(self.num_agents)]
    ...
    def _get_reward(self, agent_id):
        dist_to_bottom_left_corner = _get_dist(self.item_states[0]['pos'], (16 - 10 + 2, 12 - 10 + 2))
    ...
    def get_plan_score(self, sim_envs, agent_id, method, expected_pos=None, simulation=None, T=5):
        ...
        if method == 'action_prob': #similar actions --> high score
            if self.steps < T:
                ...
                self.trajectory_prob = np.prod([simulation['action_probs'][t]\
            ...
            plan_score = math.hypot(true_pos[0] - expected_pos[0], true_pos[1] - expected_pos[1])
        elif method == 'visibility': #visible --> high score
            plan_score = 1.0 if sim_envs[1-agent_id].in_field_of_view(1-agent_id, ('agent', agent_id)) else 0.0
        return plan_score
    ...
    def is_far(self):
        return self._get_dist_pos(self.agent_states[0]['pos'], self.agent_states[1]['pos']) > 14.14


    def get_reward_state(self, agent_id, curr_state, action, t=0, T=0, goal=None, prev_certainty=None, curr_certainty=None):
        ...
        if goal is not None and len(goal)==2:
            ...
            for g,w in zip(goal,ws):
                dist_to_goal = self._get_dist_pos(items_pos[g[1]], self.landmark_centers[g[2]])
            ...
            if goal[0] == 'LMA':
                ...
                """TODO: more objects"""
                dist_to_goal = 0.0 if attached[goal[1]] == goal[2]-self.num_agents else 1.0
            elif goal[0] == 'TEV':
                if goal[2] < self.num_agents:
                    dist_to_goal = self._get_dist_pos(agents_pos[goal[1]], agents_pos[goal[2]])
                else:
                ...
                dist_to_goal = 0.0 if self.in_field_of_view(agent_id, ('agent', goal[2])) else 1.0
            elif goal[0] == 'V':
                dist_to_goal = -self._get_dist_pos(agents_pos[goal[1]], agents_pos[goal[2]])
                dist_to_goal += self._max_dist if self.in_field_of_view(goal[2], ('agent', goal[1])) else 0.0
            elif goal[0] == 'PA':
                dist_to_goal = self._get_dist_pos(agents_pos[goal[1]], goal[2])
                dist_to_goal += self._max_dist if self.in_field_of_view(1-goal[1], ('agent', goal[1])) else 0.0
            else:
        ...
        if action in ['down', 'left', 'right', 'upleft', 'upright', 'downleft', 'downright']:
            cost += 0.02
        r -= cost
    ...
    def _setup_tmp(self, env_id, curr_state, curr_certainty=None):
        ...
        for agent_id in range(self.num_agents):
            ...
            body = self.tmp_world.CreateDynamicBody(
                position=(x,y), angle=agents_angle[agent_id],
                fixtures=b2FixtureDef(
                    shape=b2PolygonShape(vertices=[(-1.5 * R, -R),
                                                   (-0.5 * R, R),
                                                   (0.5 * R, R),
        ...
        for item_id, state in enumerate(self.tmp_item_states):
            item_pos, item_vel, item_angle = state['pos'], state['vel'], state['angle']
            self.tmp_trajectories[item_id + self.num_agents].append((item_pos[0], item_pos[1],
                                                   item_vel[0], item_vel[1], item_angle))
        ...
        for agent_id in range(self.num_agents):
            if attached[agent_id] is not None:
                ...
                dfn = b2DistanceJointDef(
                        ...
                        bodyB=self.tmp_items[attached[agent_id]], #TODO: need item id!!! #self.tmp_items[0],
                        localAnchorA=(0, 0),
                        localAnchorB=(0, 0),
                    )
                self.tmp_attached[agent_id] = self.tmp_world.CreateJoint(dfn)
                self.tmp_agents[agent_id].userData[-1] = attached[agent_id] #TODO: need item id!!!
    ...
    def _send_action_tmp(self, agent_id, action):
        ...
        if action == 'attach':
            head_mid_point = self.tmp_agents[agent_id].GetWorldPoint(localPoint=(0.0, SIZE[self.sizes[agent_id]]))
            tail_mid_point = self.tmp_agents[agent_id].GetWorldPoint(localPoint=(0.0, -SIZE[self.sizes[agent_id]]))
            right_mid_point = self.tmp_agents[agent_id].GetWorldPoint(localPoint=(SIZE[self.sizes[agent_id]], 0.0))
            left_mid_point = self.tmp_agents[agent_id].GetWorldPoint(localPoint=(-SIZE[self.sizes[agent_id]], 0.0))
            all_agent_pos = [head_mid_point, tail_mid_point, right_mid_point, left_mid_point]
            # print(all_agent_pos)
    ...
    def _get_state_tmp(self, agent_id=None):
        ...
        if agent_id is None:
            ...
            return [_get_state_from_body(agent) for agent in self.tmp_agents] + \
                   [_get_state_from_body(item) for item in self.tmp_items], \
                   [self.tmp_agents[i].certainty.copy() if i == agent_id or self.full_obs[agent_id] else np.zeros(self.room_dim) for i in range(self.num_agents)]


    def _display_imagined(self, agent_id, curr_particles, record_dir='', path_extension=''):
        ...
        self.belief_render_all_particles(agent_id, curr_particles, record_dir, path_extension)
        if self.render_separate_particles:
            self.belief_render_all_particles_separately(agent_id, curr_particles, record_dir, path_extension)
    ...
    def equivalent_states(self,state1,state2):
        ...
        return abs(round(state1['pos'][0],2) - round(state2['pos'][0],2)) <= 1 \
    ...
    def belief_matches_observation(self, agent_id, entity_id, state):
        ...
        if entity_id < self.num_agents:
            agent_grid_cells = self.get_perimeter_grid_cells('agent', entity_id, self.tmp_agents[entity_id])
            if sum([grid_cell[0]<0 or grid_cell[0]>self.room_dim[0] or grid_cell[1]<0 or grid_cell[1]>self.room_dim[1]\
                    for grid_cell in agent_grid_cells]) > 0:
                res = False
            else:
                agent_is_observable = [self.agents[agent_id].field_of_view[grid_cell[0],grid_cell[1]] \
                                       for grid_cell in agent_grid_cells]
                if sum(agent_is_observable) > 0 and\
                    not self.equivalent_states(self.agent_states[entity_id], state[entity_id]):
                    # return False, ('agent',1-agent_id)
                    res = False
        else:
            item_id = entity_id - self.num_agents
            item_grid_cells = self.get_perimeter_grid_cells('item', item_id, state[self.num_agents + item_id])
            if sum([grid_cell[0]<0 or grid_cell[0]>self.room_dim[0] or grid_cell[1]<0 or grid_cell[1]>self.room_dim[1]\
                    for grid_cell in item_grid_cells]) > 0:
                res = False
            else:
                item_is_observable = [self.agents[agent_id].field_of_view[grid_cell[0],grid_cell[1]] \
                                       for grid_cell in item_grid_cells]
                if sum(item_is_observable) > 0 and\
                    not self.equivalent_states(self.item_states[item_id], state[self.num_agents+item_id]):
                    # return False, ('item',item_id)
                    res = False
        return res


    def resample_inconsistent_entity(self, agent_id, entity_id, state, all_pos, pos_probs, fixed_shapes,
                                     max_attempts=10, num_angles=8):
        ...
        for attempt_id in range(max_attempts):
            ...
            for angle in angles:
                ...
                new_state[entity_id]['vel'] = [0.,0.]
                new_state[entity_id]['angleVel'] = 0.
                new_state[entity_id]['attached'] = None
                new_shape = self.create_shape(entity_id, new_state[entity_id])
                matches = self.belief_matches_observation(agent_id, entity_id, new_state)
                overlaps = self.overlaps(new_shape, fixed_shapes)
                if matches and not overlaps:
                    if agent_id == 1 and entity_id == 2:
                        cell = self.world_point_to_grid_cell(new_state[entity_id]['pos'][0], new_state[entity_id]['pos'][1])
    ...
    def create_shape(self, entity_id, state, scale=1.0):
        if entity_id < self.num_agents:
            return create_agent_shape(state['pos'], state['angle'], SIZE[self.sizes[entity_id]] * scale)
        else:
    ...
    def belief_update(self, agent_id, action, nb_steps=None, cInit=None, cBase=None):
        ...
        observed_entities = [int(obs[1]) + self.num_agents if obs[0] == 'item' else int(obs[1]) \
                                for obs in self.agents[agent_id].last_observations.keys()]
        observed_states = [self.item_states[obs[1]] if obs[0] == 'item' else self.agent_states[obs[1]] \
                                for obs in self.agents[agent_id].last_observations.keys()]
        observed_shapes = [self.create_shape(entity_id, gt_state, scale=1.1) \
                                for entity_id, gt_state in zip(observed_entities, observed_states)]

        room_dim, world_center = self.room_dim, self.room.position
        x,y = np.meshgrid(list(range(int(world_center[0]-(room_dim[0])//2),
                                     int(world_center[0]+(room_dim[0])//2))),\
                          list(range(int(world_center[1]+(room_dim[1])//2),
                                     int(world_center[1]-(room_dim[1])//2), -1)))
        ...
        for curr_state_idx, curr_state in enumerate(curr_particles):
            ...
            for entity_id in range(self.num_entities):
                if entity_id not in observed_entities:
                    matches = self.belief_matches_observation(agent_id, entity_id, new_particle)
                    ...
                    if matches and not overlaps:
                        ...
                        if entity_id < self.num_agents:
                            ...
                            entity_grids = self.get_perimeter_grid_cells('agent',
                                                        entity_id, body)
                        else:
                            entity_grids = self.get_perimeter_grid_cells('item',
                                                        entity_id - self.num_agents, new_particle[entity_id])
            ...
            for entity_id in range(self.num_entities):
                if entity_id not in fixed_entities:
                    new_particle[entity_id] = self.resample_inconsistent_entity(
                        agent_id, entity_id, new_particle, all_init_pos, pos_probs, fixed_shapes)
    ...
    def resample_specified_particles(self, agent_id, particle_idx2resample, goal):
        ...
        all_entities = [('agent',entity_id) for entity_id in range(self.num_agents)]+\
                        [('item',1-(self.num_agents-replace_entity_id))]
        fixed_entities = [int(obs[1]) + self.num_agents if obs[0] == 'item' else int(obs[1]) \
        ...
        for curr_state_idx in particle_idx2resample:
            ...
            fixed_states = [new_particle[self.num_agents+obs[1]] if obs[0] == 'item' else new_particle[obs[1]] \
                                    for obs in all_entities]
            fixed_shapes = [self.create_shape(entity_id, gt_state) \
    ...
    def particle_transition(self, agent_id, curr_state, action, nb_steps=None, cInit=None, cBase=None):
        """transition func (simulate one step)"""
        self._setup_tmp(self.env_id, curr_state)
    ...
    def transition(self, agent_id, curr_state, curr_certainty, action, expected_action, cInit=None, cBase=None, update_certainty=True):
        """transition func (simulate one step)"""
        ...
        self._step_tmp(update_certainty=False)#update_certainty)# and action not in [None, 'stop', 'noforce', 'attach', 'detach'])
    ...
    def set_state(self, agent_states, item_states, certainty, FOVs, steps):
        ...
        for agent_id in range(self.num_agents):
            if self.attached[agent_id] is not None:
                ...
                self.items[self.agents[agent_id].userData[-1]].userData[-1] = None #TODO item id
                self.agents[agent_id].userData[-1] = None
        ...
        for agent_id, agent_state in enumerate(agent_states):
            ...
            self.agents[agent_id].userData = [('agent',agent_id), self.touch_sensor, None]
        ...
        for agent_id in range(self.num_agents):
            ...
            # if attached[agent_id] is not None:
                ...
                dfn = b2DistanceJointDef(
                        ...
                        dampingRatio=d['rod'],
                        bodyA=self.agents[agent_id],
                        bodyB=self.items[agent_states[agent_id]['attached']], #TODO: attached item id!!! #self.items[0],
                        localAnchorA=(0, 0),
                        localAnchorB=(0, 0),
                    )
                self.attached[agent_id] = self.world.CreateJoint(dfn)
                self.agents[agent_id].userData[-1] = agent_states[agent_id]['attached'] #TODO: attached item id!!!
    ...
    def belief_render_all_particles_separately(self, render_agent_id, curr_particles, record_dir='', path_extension=''):
        ...
        for p_id, particle in enumerate(curr_particles):
            ...
            results_dir = record_dir+'/belief_snapshots/'+str(render_agent_id)+'bel'+path_extension+'/'+str(self.steps)+'agent'+str(render_agent_id)+'belief/'
            if not os.path.isdir(results_dir):
                os.makedirs(results_dir)
            plt.imsave(results_dir+str(p_id)+'.png', combined)


    def belief_render_all_particles(self, render_agent_id, curr_particles, record_dir='', path_extension=''):
        """render the tmp environment, based on render"""
        colors = self.colors
        ...
        results_dir = record_dir+'/belief_snapshots/'+str(render_agent_id)+'bel'+path_extension+'/'
        if not os.path.isdir(results_dir):
            os.makedirs(results_dir)
    ...
    def render_FOV(self, record_dir='', path_extension=''):
        ...
        for agent_id, agent in enumerate(self.agents):
            ...
            # plt.imshow(combined)
            # plt.show()
            results_dir = record_dir+'/belief_snapshots/'+str(agent_id)+'FOV'+path_extension+'/'
            if not os.path.isdir(results_dir):
                os.makedirs(results_dir)
    ...
    def render_replay(self):
        """render the environment"""
        self._setup_tmp(self.env_id, self.get_state()) #will flip angles & pos & velocity
        colors = self.colors
    ...
    def get_perimeter_grid_cells(self, entity_type, entity_id, state):
        ...
        if entity_type == 'agent':
            raw_grid_cells = []
            shape_def = [state.GetWorldPoint(localPoint=v) \
            ...
            for i,v in enumerate(int_vs):
                raw_grid_cells += list(bresenham(v[0], v[1], int_vs[(i+1)%n_Vs][0], int_vs[(i+1)%n_Vs][1]))
            # print(shape_def)
            # print(grid_cells)
            # print('bres')
            ...
            raw_grid_cells = [(R * np.cos(k*alpha) + origin[0], \
                                     R * np.sin(k*alpha) + origin[1]) for k in range(N_samples)]
    ...
    def is_center_visible(self,cell_center,agent_id):
        collisions = self.calculate_collision_walls(self.agents[agent_id].worldCenter,cell_center,agent_id)
        return len(collisions) == 0


    def _is_visible_tmp(self,cell_center,agent_id):
        collisions = self._calculate_collision_tmp(self.tmp_agents[agent_id].worldCenter,cell_center,agent_id)
        return len(collisions) == 0
    ...
    def _update_FOV(self,agent_id):
        ...
        visible_in_cone = np.array([self.is_visible(cell_center,agent_id) for cell_center in cone_world_center]).astype(int)
    ...
    def _update_FOV_tmp(self,agent_id):
        ...
        eye_pos = [((Vs[0][0] - Vs[1][0]) * 0.3 + Vs[1][0], (Vs[0][1] - Vs[1][1]) * 0.3 + Vs[1][1]),
                   ((Vs[3][0] - Vs[2][0]) * 0.3 + Vs[2][0], (Vs[3][1] - Vs[2][1]) * 0.3 + Vs[2][1])]
        p3 = self.tmp_agents[agent_id].GetWorldPoint(localPoint=(0.0,eye_pos[0][1])) #middle
        cone_mid_vec = b2Vec2(p3[0]-p1[0], p3[1]-p1[1])
    ...
    def _create_visibility_map(self):
        ...
        min_x, min_y, max_x, max_y = -self.room_dim[0]/2 + 16, self.room_dim[0]/2 + 16, -self.room_dim[1]/2 + 12, self.room_dim[1]/2 + 12
                ...
                for c1 in range(self.room_dim[1]):
                    for r2 in range(self.room_dim[0]):
                        for c2 in range(self.room_dim[1]):
                            ...
                            if _get_angle(cone_mid_vec, (p2[0]-p1[0], p2[1]-p1[1])) >= cone_angle:
                                continue
                            ...
                            for wall in maze_walls+room_walls:
                                (x, y, valid, r, s) = _get_segment_intersection(p1, p2, wall[0], wall[1])
    ...
    def in_field_of_view(self, agent_id, obs):
        ...
        if obs[0] == 'agent':
            ...
            grid_cells = self.get_perimeter_grid_cells('item', idx, self.item_states[idx])

        entity_is_observable = [self.agents[agent_id].field_of_view[grid_cell[0],grid_cell[1]] for grid_cell in grid_cells]
    ...
    def particle_in_fov(self, agent_id, state, obs):
        ...
        idx = obs[1]
        if obs[0] == 'agent':
            grid_cells = self.get_perimeter_grid_cells('agent', idx, self.tmp_agents[idx])
        elif obs[0] == 'item':
            grid_cells = self.get_perimeter_grid_cells('item', idx, self.tmp_item_states[idx])
        entity_is_observable = [self.agents[agent_id].field_of_view[grid_cell[0],grid_cell[1]] for grid_cell in grid_cells]
        return sum(entity_is_observable) > 0
    ...
    def room_in_fov(self, agent_id, room_id):
        ...
        room[x1:x2,y1:y2] = 1
        return np.sum(self.agents[agent_id].field_of_view.astype(int) & room.astype(int)) > 0 #np.sum(room)//4
    ...
    def update_observations(self):
        ...
        for agent_id, agent in enumerate(self.agents):
            ...
            for other_agent_id, other_agent_state in enumerate(self.agent_states):
                ...
                other_agent_shape = self.create_shape(other_agent_id, self.agent_states[other_agent_id], scale=1.2)
                # print(agent_shape)
                ...
                if sum(agent_is_observable) > 0 or\
                   other_agent_id == agent_id or \
                   (self.agent_states[agent_id]['attached'] == self.agent_states[other_agent_id]['attached'] is not None) or\
                   agent_shape.intersects(other_agent_shape):
                    ...
                    if other_agent_id != agent_id:
                        for item_id in range(self.num_items):
                            if self.agent_states[other_agent_id]['attached'] == item_id or\
                                    other_agent_shape.intersects(self.create_shape(self.num_agents+item_id, self.item_states[item_id], scale=1.2)):
                                agent.last_observations[('item',item_id)] = [self.item_states[item_id]['pos']]
            ...
            for item_id, item_state in enumerate(self.item_states):
                ...
                item_shape = self.create_shape(self.num_agents+item_id, self.item_states[item_id], scale=1.2)
                #perimeter in fov or item is touching or attached to agent
                if sum(item_is_observable) > 0 or \
                   agent_shape.intersects(item_shape) or \
                   self.agent_states[agent_id]['attached'] == item_id:
                    ...
                    for other_agent_id in set(range(self.num_agents))-set([agent_id]):
                        if self.agent_states[other_agent_id]['attached'] == item_id or\
                            item_shape.intersects(self.create_shape(other_agent_id, self.agent_states[other_agent_id], scale=1.2)):
                            agent.last_observations[('agent',other_agent_id)] = [self.agent_states[other_agent_id]['pos']]
                    for other_item_id in set(range(self.num_items))-set([item_id]):
                        if item_shape.intersects(self.create_shape(self.num_agents+other_item_id, self.item_states[other_item_id], scale=1.2)):
                            agent.last_observations[('item',other_item_id)] = [self.item_states[other_item_id]['pos']]
    ...
    def update_observations_human_simulation(self):
        ...
        for agent_id, agent in enumerate(self.agents):
            ...
            attached_to_same_item = agent_state['attached'] == other_agent_state['attached'] is not None
            ...
            agent_is_observable = self.is_center_visible(other_agent_pos,agent_id) and\
                                   self.world_pos_in_cone(other_agent_pos,agent_id)
            ...
            for item_id, item_state in enumerate(self.item_states):
                ...
                item_is_observable = self.is_center_visible(item_state['pos'],agent_id) and\
                                      self.world_pos_in_cone(item_state['pos'], agent_id)
                item_shape = self.create_shape(self.num_agents+item_id, self.item_states[item_id], scale=1.2)
                #perimeter in fov or item is touching or attached to agent
                # if sum(item_is_observable) > 0 or \
                if item_is_observable or \
                   agent_shape.intersects(item_shape) or \
                   self.agent_states[agent_id]['attached'] == item_id:
                    agent.last_observations[('item',item_id)] = [item_state['pos']]
                    #if this item is attached/touching other agent or touching another item, add it in
    ...
    def get_subgoals_goto(self, entity_id, goal):
        ...
        if goal[0] == 'LMA':
            if self.env_id == 0:
                if self.attached[entity_id]:
                    return ['GE', entity_id, self.num_agents+self.agent_states[entity_id]['attached'], -1]
            if self.get_reward_state(entity_id, self.get_state(), 'stop', None, None, goal) > -0.02:
                if self.attached[entity_id]:
                    return ['GE', entity_id, self.num_agents+self.agent_states[entity_id]['attached'], -1]
                else:
                    return ['stop']
        ...
        if goal[0] == 'RO':
            ...
            if entity_id < self.num_agents:
                entity_pos = self.agent_states[entity_id]['pos'] #self.agents_pos[entity_id]
            else:
                entity_pos = self.item_states[entity_id-self.num_agents]['pos'] #self.items_pos[entity_id - self.num_agents]
        ...
        if goal[0] == 'LMA':
            ...
            if goal[2] >= self.num_agents:
                goal_room = self._get_room_id(self.item_states[goal[2]-self.num_agents]['pos'])
            else:
        ...
        if path is None:
            ...
            if goal[0] in ['RO', 'LMO']:
                ...
                return ['RA', entity_id, path[0], +1] #['RA', entity_id, entity_room, +1] #TODO changed for stealing, make sure consistent with other scenarios


    def get_subgoals_avoid(self, agent_id):
        dist = self._get_dist_pos(self.agent_states[agent_id]['pos'], self.agent_states[1 - agent_id]['pos'])
        print('avoidance subgoal:', dist, 1.5 * SIZE[self.sizes[1 - agent_id]] + 1.5 * SIZE[self.sizes[agent_id]])
        if dist < 1.5 * SIZE[self.sizes[1 - agent_id]] + 1.5 * SIZE[self.sizes[agent_id]]:
        ...
        if goal[0] == 'LMA':
            ...
            if path is None:
                ...
                for door_id in doors:
                    if _get_dist(self.doors_pos[door_id], self.item_states[0]['pos']) < SIZE[self.sizes[2]] + SIZE[self.sizes[goal[1]]]: #TODO assumes item 0 is blocking
                        blocked = True
                        break
                ...
                if blocked:
                    ...
                    # input('press any key to continue...')
                    return self.get_subgoals_put(agent_id, ['LMO', 0, item_room, +1]) #TODO assumes item 0 is blocking
                    ...
                    if self.attached[agent_id] is not None:
                        return ['GE', agent_id, self.num_agents+self.agent_state[agent_id]['attached'], -1] # detach
                    else:
                        return ['stop']
        elif goal[0] == 'TE': # touch entity
            if goal[2] >= self.num_agents:
                item_room = self._get_room_id(self.item_states[goal[2]-self.num_agents]['pos'])
                print('gotohelp:', entity_room, agent_room, item_room)
                if entity_room == item_room and self.get_reward_state(goal[1], self.get_state(), 'stop', None, None, goal) > -0.1:
                    if self.attached[agent_id] is not None:
                        return ['GE', agent_id, goal[2], -1] # release
                    else:
                        return ['stop'] # stop
                # goal_room = self._get_room_id(self.items_pos[0])
                if self.attached[agent_id] is not None:# or self._get_dist_pos(self.agents_pos[agent_id], self.items_pos[0]) < SIZE[self.sizes[agent_id]] + SIZE[self.sizes[2]] + 0.2:
                    if agent_room == entity_room: # in the same room
                        return goal
                    else:
                        return self.get_subgoals_goto(goal[2], ['RO', goal[2]-self.num_agents, entity_room, +1]) # approach the other agent
                else:
                    if self.once_attached[agent_id] and agent_room == item_room: # TODO: more general
                        return self.get_subgoals_goto(goal[2], ['RO', goal[2]-self.num_agents, entity_room, +1])
                    else:
                        return self.get_subgoals_put(agent_id, ['LMO', goal[2]-self.num_agents, entity_room, +1]) # get the item first
            else:
                if self.get_reward_state(goal[1], self.get_state(), 'stop', None, None, goal) > -0.1:
                    if self.attached[agent_id] is not None:
                        return ['GE', agent_id, self.num_agents+self.agent_state[agent_id]['attached'], -1]
                    #TODO added for informing, check that doesn't change other scenarios.
                    else:
                        return ['stop'] # stop
                return self.get_subgoals_goto(agent_id, ['TE', agent_id, 1-agent_id, +1])
        return goal
        ...
        if goal[0] == 'LMA':
            ...
            if entity_room == goal_room:
                if self.attached[agent_id]:
                    return ['GE', agent_id, self.num_agents+self.agent_states[agent_id]['attached'], -1]
                if agent_room != goal_room:
                    return self.get_subgoals_goto(agent_id, ['LMA', agent_id, goal_room, +1])
                else:
                    return [goal[0], goal[1], goal[2], -1]
            path = self.path[(entity_room, goal_room)]
            ...
            if with_object:
                ...
                for room_id in range(len(path)):
                    ...
                    if self.doors_size[door] < 2 * SIZE[self.sizes[2]]: #TODO 1 item
                        ...
                        if item_room == room_id1:
                            return self.get_subgoals_put(agent_id, ['LMO', 0, room_id2, +1]) #TODO 1 item
                        if item_room == room_id2:
                            return self.get_subgoals_put(agent_id, ['LMO', 0, room_id1, +1]) #TODO 1 item
                        path1 = self.path[(item_room, room_id1)]
                        path2 = self.path[(item_room, room_id2)]
                        if path1 is None and path2 is None: continue
                ...
                if best_room is not None: # check if it is too far
                    len2item = 0 if agent_room == item_room or self.attached[agent_id] else len(self.path[(agent_room, item_room)])
                    len2door = 0 if item_room == best_room else len(self.path[(item_room, best_room)])
                    # print('len:', len2item, len2door, len(path))
                    """TODO: use distance instead?"""
                    if len2item + len2door < len(path):
                        return self.get_subgoals_put(agent_id, ['LMO', 0, best_room, +1]) #TODO 1 item
            ...
            for i, room_id in enumerate(all_path):
                if i < len(all_path) - 1:
                    if self.doors_size[self.connected[all_path[i]][all_path[i+1]]] >= 10 - 1e-6: # if no wall at all
                        continue
                if agent_room == room_id: continue#return 12 + goal_id
            ...
            if self.env_id == 0:
                if not self.attached[agent_id]:
                    return self.get_subgoals_put(agent_id, ['LMO', item_id, item_room, +1])
                else:
            ...
            if agent_room != item_room and self._is_blocked(self.doors[(agent_room, item_room)], SIZE[self.sizes[agent_id]]):
                if self.attached[goal_agent] or entity_room == item_room:
                    return ['stop'] # there is nothing it can do
                return self.get_subgoals_goto_hinder(agent_id, ['RA', goal_agent, item_room, +1], with_object=False) # try to block by itself
            if self.attached[goal_agent]:
                ...
                for room_id in range(4):
                    if entity_room != room_id and item_room != room_id:
                        if self._is_blocked(self.doors[(entity_room, room_id)], SIZE[self.sizes[goal_agent]]) \
                            and not self._is_blocked(self.doors[(item_room, room_id)], SIZE[self.sizes[item_id]]) \
                            and (agent_room == room_id \
                                or not self._is_blocked(self.doors[(agent_room, room_id)], SIZE[self.sizes[agent_id]])):
                            path = self.path[(item_room, room_id)]
                            if best_room is None or len(path) < best_path:
                                best_room = room_id
                                best_path = len(path)
                if best_room is not None:
                    return self.get_subgoals_put(agent_id, ['RO', item_id, best_room, +1]) # hind in a room
                ...
                if len(path) > 1 and agent_room in path[:-1]: # in the path of opponent
                    return self.get_subgoals_goto_hinder(agent_id, ['RA', goal_agent, room_id, +1], with_object=False) # block by itself
                    ...
                    if not self.attached[agent_id]:
                        ...
                        for room_id in range(4):
                            if room_id != entity_room and room_id != item_room:
                                entity_dist = self._get_dist_room(self.agent_states[goal_agent]['pos'], room_id)
                                item_dist = self._get_dist_room(self.item_states[item_id]['pos'], room_id)
                                if item_dist < entity_dist and item_dist > best_dist:
                                    best_dist = item_dist
                                    best_room = room_id
                        if best_room is not None:
                            return self.get_subgoals_goto(self.num_agents+item_id, ['RO', item_id, best_room, +1])
                        return ['TE', goal_agent, self.num_agents+item_id, -1]
    ...
    def get_cost(self, agent_id, goal):
        if goal[0] == 'LMA':
            ...
            return self._get_dist_pos(self.agent_states[goal[1]]['pos'], self.landmark_centers[goal[2]])
        if goal[0] == 'LMO':
            ...
            # return self._get_dist_pos(self.item_states[goal[1]]['pos'], self.landmark_centers[goal[2]]) + \
            return self._get_dist_pos(self.item_states[goal[1]]['pos'], self.agent_states[agent_id]['pos'])
        return 0
    ...
    def get_subgoals_put(self, agent_id, goal):
        ...
        if self.env_id == 0:
            if self.attached[agent_id] is None:
                if self.get_reward_state(agent_id, self.get_state(), 'stop', None, None, goal) > -0.05:
                    return ['stop'] # stop
                ...
                #     return self.get_subgoals_goto(agent_id, ['TE', agent_id, self.num_agents+goal[1], +1]) # approach the object
            elif self.get_reward_state(agent_id, self.get_state(), 'stop', None, None, goal) > -0.05 \
                and not self.once_attached[agent_id]: #TODO: get rid of once_attached!
        ...
        if self.attached[agent_id] is not None and \
           (self.attached[1 - agent_id] is not None or self.num_agents < 2):
            ...
            if agent_room != goal_room:
                ...
                if blocked:
                    return ['GE', agent_id, self.num_agents+goal[1], -1] #TODO not sure goal[1] has item
        ...
        if item_room == goal_room and agent_room == goal_room:
            ...
            if agent_room == goal_room and self.agent_states[agent_id]['attached'] != goal[1]:
    ...
    def get_subgoals_put_help(self, agent_id, goal):
        ...
        if self.get_reward_state(agent_id, self.get_state(), 'stop', None, None, goal) > -0.05:
            if self.attached[agent_id]:
                return ['GE', agent_id, self.agent_states[agent_id]['attached']+self.num_agents, -1] #56 + agent_id
            else:
                return ['stop'] # TODO: stop only once
        goal_agent = 1 - agent_id
        entity_pos = self.agent_states[goal_agent]['pos']
        entity_room = self._get_room_id(entity_pos)
        item_room = self._get_room_id(self.item_states[goal[1]]['pos']) #for 2 items assuming LMO. previously - self.items_pos[0]
        agent_room = self._get_room_id(self.agent_states[agent_id]['pos'])
        goal_room = goal[2]
        ...
        if agent_room != item_room:
            ...
            agent_item_blocked = self._is_blocked(doors, SIZE[self.sizes[agent_id]] * 1.5)
        entity_goal_blocked = False
        if entity_room != goal_room:
            path = self.path[(entity_room, goal_room)]
            doors = self.doors[(entity_room, goal_room)]
            entity_goal_blocked = self._is_blocked(doors, SIZE[self.sizes[goal_agent]] * 1.5)
        entity_item_blocked = False
        if entity_room != item_room:
            path = self.path[(entity_room, item_room)]
            doors = self.doors[(entity_room, item_room)]
            entity_item_blocked = self._is_blocked(doors, SIZE[self.sizes[goal_agent]] * 1.5)
        ...
        if self.attached[agent_id] is not None and \
           self.attached[goal_agent] is not None:
    ...
    def get_subgoals_put_hinder(self, agent_id, goal):
        ...
        if self.env_id == 4 and self.sizes[agent_id] == 0 and self.sizes[self.num_agents+goal[1]] == 0:
            if entity_room != item_room and self._is_blocked(self.doors[(entity_room, item_room)], SIZE[self.sizes[agent_id]]):
        ...
        if self.attached[goal_agent]: # in the process of pushing the object
            ...
            # check if achievable
            if entity_room != goal_room and self._is_blocked(self.doors[(entity_room, goal_room)], SIZE[self.sizes[goal_agent]]) \
               or item_room != goal_room and self._is_blocked(self.doors[(item_room, goal_room)], SIZE[self.sizes[goal_agent]]):
                if self.attached[agent_id]:
                    return ['GE', agent_id, self.num_agents+self.agent_states[agent_id]['attached'], -1]
                else:
                    return ['stop']
        ...
        for room_id in range(4):
            if entity_room != room_id and item_room != room_id:
                if self._is_blocked(self.doors[(entity_room, room_id)], SIZE[self.sizes[goal_agent]]) \
                    and not self._is_blocked(self.doors[(item_room, room_id)], SIZE[self.sizes[self.num_agents+goal[1]]]) \
                    and (agent_room == room_id \
                        or not self._is_blocked(self.doors[(agent_room, room_id)], SIZE[self.sizes[agent_id]])):
                    return self.get_subgoals_put(agent_id, ['RO', goal[1], room_id, +1]) # hide in a room
        # if entity_room == item_room:
        #     if not self.attached[agent_id]:
        ...
        if not self.attached[agent_id]:
            ...
            for room_id in range(4):
                if room_id != entity_room and room_id != item_room:
                    entity_dist = self._get_dist_room(self.agent_states[goal_agent]['pos'], room_id)
                    item_dist = self._get_dist_room(self.item_states[goal[1]]['pos'], room_id)
                    if item_dist < entity_dist and item_dist > best_dist:
                        best_dist = item_dist
                        best_room = room_id
            if best_room is not None:
                return self.get_subgoals_goto(self.num_agents+goal[1], ['RO', goal[1], best_room, +1])
            return ['LMA', goal_agent, entity_room, -1] #['LMA', goal_agent, 0, -1] #TODO why 0 and not remove from entity_room / item_room?

        return ['LMA', goal_agent, entity_room, -1] #['LMA', goal_agent, 0, -1] #TODO why 0?
    ...
    def multiple_goals(self, goals, agent_id):
        ...
        if _get_dist(self.item_states[goal1_item]['pos'], self.landmark_centers[goal1_landmark]) > 2.5:
        ...
        elif self._get_room_id(self.agent_states[1-agent_id]['pos']) == \
                self._get_room_id(self.item_states[goal1_item]['pos']) == goal1_landmark:
            return ['LMA', 1-agent_id, goal1_landmark, -1] #TODO prevent touching - if actually see green call goto_hinder decompos + change all particle decom - if exists 1 goto
        #goal2
        else:
            if self.num_agents == 2:
                ...
                if room_id == goal1[2] or visible:
                    if visible:
                        return self.get_subgoals_goto_hinder(agent_id, ['LMA', 1 - agent_id, room_id, +1], with_object=False)
                    else:
                        return self.get_subgoals_goto(agent_id, ['TE', agent_id, 1 - agent_id, +1])
            print('object 2')
